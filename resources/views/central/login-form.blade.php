...
<div class="p-8 md:p-16 xl:p-24 py-24  min-w-site text-sm font-medium min-h-full text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-900 relative">
    ...
    <div class="mx-auto w-full max-w-2xl  grid place-items-center py-4">
        ...
        <form action="{{ route('user.login') }}" method="POST" class="w-full bg-white dark:bg-gray-800 p-8 rounded-lg shadow-md">
            @csrf
            ...
            <div class="flex items-center justify-between">
                <button type="submit" class="bg-blue-600 cursor-pointer duration-200 transition-all hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none
    ...
    <div class="mx-auto w-full max-w-2xl py-4 flex flex-col gap-4">
        <a class="hover:text-blue-600 duration-200 transition-colors" href="{{route('central.forgot-password')}}"> Forgot your password? </a>
        
        <a class="hover:text-blue-600 duration-200 transition-colors" href="{{route('central.register')}}"> Register Account </a>
    </div>
